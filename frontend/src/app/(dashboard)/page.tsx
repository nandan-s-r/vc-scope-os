...
function ErrorBanner({ message }: { message: string }) {
  return (
    <div className="mono" style={{ fontSize: '11px', color: 'var(--accent-red)', padding: '8px 12px', background: 'rgba(239, 68, 68, 0.1)', borderRadius: '4px', border: '1px solid rgba(239, 68, 68, 0.2)' }}>
      [ERR] {message}
    </div>
  );
}
...
function RiskBadge({ level }: { level: string }) {
  ...
  return (
    <span className="mono" style={{ fontSize: '10px', padding: '2px 6px', background: s.bg, color: s.color, borderRadius: '4px', fontWeight: 600, letterSpacing: '0.02em' }}>
  ...
  return (
    <>
      ...
      <div className="dashboard-grid">
            ...
            {alerts.length > 0 ? alerts.slice(0, 3).map((a: any, i: number) => (
              <div key={a.id ?? i} className="fade-in" style={{ padding: '12px 0', borderBottom: '1px solid var(--border-subtle)' }}>
                <div className="flex-between" style={{ marginBottom: '4px' }}>
                  {a.startup_id ? (
                    <Link href={`/startups/${a.startup_id}`} style={{ textDecoration: 'underline', textDecorationColor: 'transparent' }} onMouseEnter={e => e.currentTarget.style.textDecorationColor = 'var(--text-primary)'} onMouseLeave={e => e.currentTarget.style.textDecorationColor = 'transparent'}>
                      <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)' }}>{a.startup_name}</span>
                    </Link>
                  ) : (
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)' }}>{a.startup_name}</span>
                  )}
                ...
                <div className="mono text-muted" style={{ fontSize: '11px', marginBottom: '6px' }}>{a.event_type ?? 'Monitoring Event'}</div>
                {a.ai_summary && (
                  <div className="text-secondary" style={{ fontSize: '12px', lineHeight: 1.5 }}>
                    {a.ai_summary.slice(0, 80)}{a.ai_summary.length > 80 ? '...' : ''}
                  </div>
                )}
              </div>
            )) : (
          ...
          <div className="panel" style={{ borderLeft: '3px solid var(--accent-amber)' }}>
            ...
            {portcos.length > 0 ? (
              <table className="data-table">
                ...
                <tbody>
                  {portcos.slice(0, 5).map((p: any, i: number) => (
        ...
        {/* ═══ CENTER COLUMN: INTELLIGENCE FEED ═══════════════════════════════ */}
        <div className="col-stack">
          ...
          <div className="panel panel-elevated fade-in" style={{ borderLeft: '3px solid var(--accent-blue)', padding: '24px' }}>
            <div className="flex-between" style={{ marginBottom: '16px' }}>
              <div className="tag" style={{ background: 'rgba(59,130,246,0.1)', color: 'var(--accent-blue)' }}>AI Deal Copilot • Sourcing</div>
              <span className="mono text-muted">14:22 EST</span>
            </div>
            <h3 style={{ fontSize: '15px', marginBottom: '8px' }}>
              <Link href="/startups/1" style={{ color: 'inherit', textDecoration: 'underline', textDecorationColor: 'transparent' }} onMouseEnter={e => e.currentTarget.style.textDecorationColor = 'var(--text-primary)'} onMouseLeave={e => e.currentTarget.style.textDecorationColor = 'transparent'}>
                Valuation Misalignment Detected: DataScale Series A
              </Link>
            </h3>
          ...
          <div className="panel fade-in" style={{ borderLeft: '3px solid var(--accent-emerald)', padding: '24px' }}>
            <div className="flex-between" style={{ marginBottom: '16px' }}>
              ...
              <span className="mono text-muted">11:05 EST</span>
            </div>
            <h3 style={{ fontSize: '15px', marginBottom: '8px' }}>
              <Link href="/startups/2" style={{ color: 'inherit', textDecoration: 'underline', textDecorationColor: 'transparent' }} onMouseEnter={e => e.currentTarget.style.textDecorationColor = 'var(--text-primary)'} onMouseLeave={e => e.currentTarget.style.textDecorationColor = 'transparent'}>
                NeuroFlow AI
              </Link>
            </h3>
            <p className="text-secondary" style={{ fontSize: '13px', marginBottom: '16px', lineHeight: 1.6 }}>
          ...
          {meetings.slice(0, 2).map((m: any) => (
            <div key={m.id} className="panel fade-in" style={{ borderLeft: '3px solid var(--accent-violet)', padding: '24px' }}>
              <div className="flex-between" style={{ marginBottom: '16px' }}>
                <div className="tag" style={{ background: 'rgba(99,102,241,0.1)', color: 'var(--accent-violet)' }}>
                  Meeting • {m.meeting_type ?? 'Pitch'}
                </div>
              ...
              <h3 style={{ fontSize: '15px', marginBottom: '8px' }}>
                {m.startup_id ? (
                  <Link href={`/startups/${m.startup_id}`} style={{ color: 'inherit', textDecoration: 'underline', textDecorationColor: 'transparent' }} onMouseEnter={e => e.currentTarget.style.textDecorationColor = 'var(--text-primary)'} onMouseLeave={e => e.currentTarget.style.textDecorationColor = 'transparent'}>
                    {m.startup_name}
                  </Link>
                ) : (
                  m.startup_name
              ...
              {m.ai_summary && (
                <p className="text-secondary" style={{ fontSize: '13px', lineHeight: 1.6 }}>{m.ai_summary}</p>
              )}
            </div>
          ))}
          ...
          <div className="panel" style={{ background: '#090A0C', border: '1px solid var(--border-subtle)' }}>
            ...
            <div style={{ marginBottom: '20px' }}>
              <div className="telemetry-row">
                <span className="mono text-secondary">Database Connection</span>
              ...
              <div className="telemetry-row">
                <span className="mono text-secondary">Background Queue Size</span>
                <span className="mono text-primary">2 jobs</span>
              </div>
            </div>
            ...
            <div style={{ background: '#000', padding: '12px', borderRadius: '4px', height: '140px', overflowY: 'auto', border: '1px solid var(--border-subtle)' }}>
              ...
              <div className="log-line"><span className="log-timestamp">21:24:18</span><span style={{ color: 'var(--accent-emerald)' }}>[SYNC]</span><span>Crunchbase delta synced (42 ms).</span></div>
              ...
              <div className="log-line"><span className="log-timestamp">21:27:00</span><span style={{ color: 'var(--accent-emerald)' }}>[SYNC]</span><span>Portfolio monitor health check passed.</span></div>
          ...
          <div className="panel">
            ...
            {meetings.length > 0 ? meetings.slice(0, 2).map((m: any) => (
              <div key={m.id} style={{ padding: '12px 0', borderBottom: '1px solid var(--border-subtle)' }}>
                ...
                <button className="btn" style={{ width: '100%' }}>Resume Workflow</button>
              </div>
            )) : (
            ...
            {outreach.length > 0 ? outreach.slice(0, 3).map((e: any) => (
              <div key={e.id} style={{ padding: '12px 0', borderBottom: '1px solid var(--border-subtle)' }}>
                <div className="flex-between" style={{ marginBottom: '6px' }}>
                  <span style={{ fontSize: '13px', fontWeight: 600 }}>
                    {e.startup_id ? (
                      <Link href={`/startups/${e.startup_id}`} style={{ color: 'inherit', textDecoration: 'underline', textDecorationColor: 'transparent' }} onMouseEnter={e => e.currentTarget.style.textDecorationColor = 'var(--text-primary)'} onMouseLeave={e => e.currentTarget.style.textDecorationColor = 'transparent'}>
                        {e.startup_name}
                      </Link>
                    ) : (
                      e.startup_name
